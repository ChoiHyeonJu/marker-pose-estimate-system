...
class OMOR1MiniNode(Node):
  def __init__(self):
    ...
    self.declare_parameters(
      namespace='',
      parameters=[
        ('port.name', '/dev/ttyMCU'),
        ('port.baudrate', 115200),
        ('wheel.separation', 0.17),
        ('wheel.radius', 0.0335),
        ('motor.gear_ratio', 21.3),
        ('motor.max_lin_vel_x', 0.425),
        ('motor.max_ang_vel_z', 5.0),
        ('sensor.enc_pulse', 1440.0),
        ('sensor.use_gyro', True),
        ('odometry.odom_publish', True),
        ('odometry.tf_publish', True),
        ('odometry.topic_name', 'omo_odom'),
        ('odometry.frame_id', 'odom'),
        ('odometry.child_frame_id', 'base_link')
      ])
    # Get parameter values
    _port_name = self.get_parameter('port.name').get_parameter_value().string_value
    _port_baudrate = self.get_parameter('port.baudrate').get_parameter_value().integer_value
    self.gear_ratio = self.get_parameter('motor.gear_ratio').get_parameter_value().double_value
    self.wheel_separation = self.get_parameter('wheel.separation').get_parameter_value().double_value # 0.085 cm x 2
    self.wheel_radius = self.get_parameter('wheel.radius').get_parameter_value().double_value

    self.max_lin_vel_x = self.get_parameter('motor.max_lin_vel_x').get_parameter_value().double_value
    self.max_ang_vel_z = self.get_parameter('motor.max_ang_vel_z').get_parameter_value().double_value

    self.enc_pulse = self.get_parameter('sensor.enc_pulse').get_parameter_value().double_value
    self.use_gyro = self.get_parameter('sensor.use_gyro').get_parameter_value().bool_value

    self.odom_publish = self.get_parameter('odometry.odom_publish').get_parameter_value().bool_value
    self.tf_publish = self.get_parameter('odometry.tf_publish').get_parameter_value().bool_value
    self.odom_topic_name = self.get_parameter('odometry.topic_name').get_parameter_value().string_value
    self.odom_frame_id = self.get_parameter('odometry.frame_id').get_parameter_value().string_value
    self.odom_child_frame_id = self.get_parameter('odometry.child_frame_id').get_parameter_value().string_value

    print('MAX V:\t\t%s'%(self.max_lin_vel_x))
    print('MAX W:\t\t%s'%(self.max_ang_vel_z))
    ...
    print('Odom topic: %s'%(self.odom_topic_name))
    ...
    if self.odom_publish:
      print('[Publish odom] True')
    else:
      print('[Publish odom] False')

    if self.tf_publish:
      print('[Publish tf odom] True')
    else:
      print('[Publish tf odom] False')
    ...
    if self.odom_publish:
      ...
      self.pub_Odom = self.create_publisher(Odometry, self.odom_topic_name, 10)
    if self.tf_publish:
